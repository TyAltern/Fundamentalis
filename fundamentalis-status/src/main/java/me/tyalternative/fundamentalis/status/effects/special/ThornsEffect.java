...
public class ThornsEffect extends StatusEffect implements Listener {

    // -------------------------------------------------------------------------
    // Constantes
    // -------------------------------------------------------------------------

    private static final double THORNS_RATIO_PER_LEVEL = 0.10; // +10% de thorns par niveau
    ...
    public ThornsEffect(ComponentHolder holder, IStatsComponent statsComponent, ActiveStatusEffect meta,
    ...
    public void onPostDamage(PostDamageEvent event) {
        ...
        if (!victim.getUniqueId().equals(getEntity().getUniqueId())) return;
        LivingEntity attacker = event.getAttacker();
        if (attacker == null) return;
        double thornsDamage = event.getFinalDamage() * THORNS_RATIO_PER_LEVEL * getLevel();

        DamageInfo info = damageManager.createStatusDamage(
                attacker, DamageSource.STATUS_THORNS, thornsDamage, DamageType.PHYSICAL);
